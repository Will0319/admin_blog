...
import { Button, notification, Card, Row, Col, Form, Icon, Input } from 'antd';
...
class Login extends Component {
    ...
    handleSubmit = (e) => {
        ...
        this.props.form.validateFields((err, values) => {
            if (!err) {
                POST('/api/user/login', { name: values.name, pwd: md5(values.pwd)}).then(res=>{
                    this.setState({ loading: false })
                    const data = res.data;
                    if (data.error_code == 0) {
                        // 本地存储用户信息
                        localStorage.setItem("will_blog_user", JSON.stringify(data.user_info));
                        localStorage.setItem("will_blog_user_token", data.token);
    ...
    render() {
        ...
        return (
            <Row className='Login-bg'>
                <Col span={6}>
                    <Card
                        title={<div style={{ textAlign: 'center' }}>Will Blog <PandaIcon  /></div>}
                    >
